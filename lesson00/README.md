...
| int64      | 有符号 64 位整型 (-9223372036854775808 到 9223372036854775807), 但是32位系统中会退化为int32 |
...
```
### 作用域
Go语言中的局部变量（在{}中声明的变量）都有自己的作用域，通常在`{}`内。
```go
func Name(){
    var a = "first"
    var b,c =1,2
    if b==1{
        c = c + 1 //大的{}可以包含小的{},所以c可以在更小的if{}中使用
        var d = 3 //在if{}中声明
    }
    fmt.Printf("%d",d) //报错，因为d超出了if{}的作用域，对于func{}d是未声明的变量 
}
```
后面会讲到的for{}和if{},可在开头的部分声明变量,作用域在for{},if{}之内。
```go
for i:=0;i<5;i++{
	fmt.Print(i)//打印01234 
}
...
fmt.Print(b)//报错，b未声明
```
但是`:=`有一个很容易踩的坑，在次级作用域中使用:=创建多个变量时，上级作用域的变量也会被重新声明的次级作用域的同名变量覆盖
```go
func Name(){
    var a = 1
    {
    ...
    fmt.Println(a) //输出 1 
    // 因为在{}中b是新的变量，并且{}中没有a的声明，编译器自动创建了一个更小作用域的，新的变量a
}
```
### 运算符
